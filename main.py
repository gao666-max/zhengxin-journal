...
记账：如果她提到花了多少钱、赚了多少钱、买了什么、收到了什么钱，就记进「记账」数组。类型只能是「支出」或「收入」，金额是数字（不带货币符号）。分类从这些里选最贴切的——支出：餐饮/交通/购物/娱乐/居住/通讯/医疗/学习/人情/其他；收入：工资/兼职/红包/其他。她没提到钱就不要记。绝对不要在回复里心算或报出总支出/总收入（你算总数经常出错），你只逐笔提取每一笔金额，总额由系统自动求和。
